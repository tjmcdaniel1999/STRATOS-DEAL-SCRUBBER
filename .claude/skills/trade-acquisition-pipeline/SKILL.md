...
expects (they map directly to the ZoomInfo calls above).

Deliver as `.xlsx`. The user converts it to a Google Sheet themselves by dragging it into
Drive (this preserves formatting better than pushing it through the Drive API as base64).

## 9. Report next actions, in priority order

Always end a pipeline run with a short punch list, e.g.:

1. Enrich the N callable owners (~N ZI credits, near-100% expected conversion).
2. Name the remaining unnamed owners via the state licensing-board lookup.
3. Deepen the pool if only a small fraction of the statewide universe in the target headcount
   band has been pulled so far — company search is free, just paginate further.
4. Offer to backfill the DNC column on any older state sheets that predate this protocol
   (e.g., an original list built before the DNC sweep existed) — it's a free sweep and a real
   liability reduction.
5. Flag Apollo status (dead/usable) so it doesn't sit around silently assumed-working.

## 10. Adapting to a new state

Everything above is state-agnostic except two things:
- **Licensing-board reference** — different state, different agency and license-prefix
  scheme. Look it up in `references/licensing_boards.md`; if the state isn't listed yet, find
  the state's contractor licensing board's public license-lookup tool and add it to that file
  so the next run doesn't redo the research.
- **Roll-up blocklist** — the national platforms in `references/rollup_blocklist.md` apply
  everywhere, but each state also has regional roll-ups the user may know about that aren't in
  the seed list yet. Ask, and add anything confirmed to the blocklist file for next time.
